...
namespace Alkahest.Core.Net.Game.Serialization
{
    public sealed class ReflectionPacketSerializer : PacketSerializer
    {
        sealed class ReflectionPacketFieldInfo : PacketFieldInfo
        {
            ...
            public Action<GameBinaryWriter, object> PrimitiveSerializer { get; }

            public Func<GameBinaryReader, object> PrimitiveDeserializer { get; }
                ...
                if (type.IsEnum && type.GetCustomAttribute<FlagsAttribute>() == null)
                    EnumValidator = val =>
                    {
                        if (!Enum.IsDefined(type, val))
                            _log.Warning("Found unknown enum value {0} for field {1}.{2} ({3})", val,
                                property.DeclaringType.Name, property.Name, type.Name);
                    };
                ...
                out Action<GameBinaryWriter, object> serializer, out Func<GameBinaryReader, object> deserializer)
            {
                ...
                {
                    serializer = (w, v) => w.WriteDouble((double)v);
                    deserializer = r => r.ReadDouble();
                }
                ...
                else if (type == typeof(Appearance))
                {
                    serializer = (w, v) => w.WriteAppearance((Appearance)v);
                    deserializer = r => r.ReadAppearance();
                }
                else
